...
from util import load_dict, get_config, load_documents, count_true_answers, get_question_index
from data_generator import DataLoader


# inbatch
def get_feature_inbatch(feed, ent_emb, q_node_emb):

    answers = feed['answers'].float()
    batch_size = answers.size(0)
    flag = 2 if batch_size == 8 else 4
    _, answer_index = count_true_answers(answers, batch_size, flag=batch_size//flag)
    _, question_index = get_question_index(q_node_emb, answer_index)
    if len(answer_index) != 0:
        answer_feature = [ent_emb[k[0], k[1], :] for k in answer_index]
        question_feature = [q_node_emb[k[0], k[1], :] for k in question_index]
        answer_features = torch.stack(answer_feature, 0)
        question_features = torch.stack(question_feature, 0)

        feature = torch.cat([answer_features, question_features], dim=1)
    else:
        feature = torch.zeros(2, ent_emb.size(2) * 2)
    return feature


# moco
def get_feature_moco(ent_emb, q_node_emb, index):
    _, question_index = get_question_index(q_node_emb, index)
    if len(index) != 0:
        answer_feature = [ent_emb[k[0], k[1], :] for k in index]
        question_feature = [q_node_emb[k[0], k[1], :] for k in index]
        answer_features = torch.stack(answer_feature, 0)
        question_features = torch.stack(question_feature, 0)

        feature = torch.cat([answer_features, question_features], dim=1)
    else:
        feature = torch.zeros(1, ent_emb.size(2) * 2)
    return feature
...
class KAReader(nn.Module):
    ...
    def forward(self, feed, index=None, mode='train', mlp=False):
        ...
        features = []
        if mode == 'train':
            features = get_feature_moco(ent_emb, q_node_emb, index) if index else get_feature_inbatch(feed, ent_emb, q_node_emb)
            if mlp:
        ...
        return loss, pred, pred_dist, features
...
if __name__ == '__main__':
    ...
    for feed in batcher:
        loss, pred, pred_dist, ent_emb, q_node_emb = model(feed)
